...
func (c *operationContext) startOperationOutcome(
	...
	case commonnexus.DispatchOutcomeOperationFailureDeprecated:
		//nolint:staticcheck // Deprecated function still in use for backward compatibility.
		cause := &nexus.FailureError{
			Failure: commonnexus.ProtoFailureToNexusFailure(result.OperationError.GetFailure()),
		}
...
func (c *operationContext) dispatchFailureToNexusError(
	...
	case commonnexus.DispatchOutcomeHandlerFailureDeprecated:
		return commonnexus.ProtoHandlerErrorToNexusHandlerError(result.HandlerError)
